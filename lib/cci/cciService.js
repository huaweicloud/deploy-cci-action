...
function updateDeployment(inputs) {
    return __awaiter(this, void 0, void 0, function* () {
        if (inputs.manifest) { // 根据用户yaml文件更新Deployment
            ...
            // 获取镜像名称
            const result = yield (cp.execSync(`kubectl get deployment ${inputs.deployment} -o=jsonpath='{$.spec.template.spec.containers[*].name}' -n ${inputs.namespace}`) || '').toString();
            // 更新镜像
            yield (cp.execSync(`kubectl set image deploy ${inputs.deployment} ${result}=${inputs.image} -n ${inputs.namespace}`) || '').toString();
        }
        // 新建vip
        const publicipId = yield eip.createPublicip();
        const subnetID = yield getCCINetworkSubnetID(inputs);
