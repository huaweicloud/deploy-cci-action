...
function updateDeployment(inputs) {
    return __awaiter(this, void 0, void 0, function* () {
        if (inputs.manifest) { // 根据用户yaml文件更新Deployment
            ...
            // 获取镜像名称
            const result = yield utils.execCommand("kubectl get deployment " + inputs.deployment + " -o=jsonpath='{$.spec.template.spec.containers[0].name}' -n " + inputs.namespace);
            // 更新镜像
            yield utils.execCommand("kubectl set image deploy " + inputs.deployment + " " + result + "=" + inputs.image + " -n " + inputs.namespace);
        }
        // 新建vip
        const publicipId = yield eip.createPublicip(inputs);
        const subnetID = yield getCCINetworkSubnetID(inputs);
...
function isDeploymentExist(inputs) {
    return __awaiter(this, void 0, void 0, function* () {
        let isExist = false;
        try {
            const result = yield utils.execCommand("kubectl get deployment -n " + inputs.namespace + " | awk '{if (NR > 1) {print $1}}'");
